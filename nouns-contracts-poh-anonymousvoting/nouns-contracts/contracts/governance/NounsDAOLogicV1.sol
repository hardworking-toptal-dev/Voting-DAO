...
import './ProofOfHumanityProxy.sol';
contract NounsDAOLogicV1 is NounsDAOStorageV1, NounsDAOEvents {
     ...
     * @param proofofhumanityproxy_ The address of proof of humanity proxy contract
    ...
    function initialize(
        ...
        vetoer = vetoer_;
        proofofhumanityproxy = IProofOfHumanity(proofofhumanityproxy_);
    ...
    function propose(
        ...
        if(poh == 1){
            require(proofofhumanityproxy.isRegistered(msg.sender),'NounsDAO::propose: proposer not a human');
        }
        require(
    ...
    function castVoteInternal(
        ...
        if(proposal.poh == 1){
            require(proofofhumanityproxy.isRegistered(voter),'NounsDAO::propose: voter not a human');
        }
    ...
    /**
     * @notice Admin function for setting the poh contract
     */
    function _setPohContract(address proofofhumanityproxy_) external {
        require(msg.sender == admin, 'NounsDAO::_setVotingPeriod: admin only');
        require(
            proofofhumanityproxy_ != address(0),
            'NounsDAO::_setVotingPeriod: invalid proof of humanity proxy contract period'
        );
        proofofhumanityproxy = IProofOfHumanity(proofofhumanityproxy_);
    }
